...
corners = cv2.goodFeaturesToTrack(gray_img, 200, 0.01, img_max_height/50.0)
...
#         if d < img_max_height/80.0 or d > img_max_height/8.0:
#           bad_set = True
#           break
#     if bad_set:
#       continue
#     ordered = cv2.convexHull(srcpts)
...
#   srcpts = cv2.convexHull(srcpts, clockwise=True)[:,0,:]
#   # order start indices bottom-left clockwise around the square
#   # sides = srcpts.argsort(0)
...
#   if M1[0,0] < 0.005 or M1[1,1] < 0.005:
#     continue
#   # print(M1)
...
#   # Find consensus
#   consensus = np.sum((warpedpts[:,:2] - warpedpts[:,:2].round()).A **2, 1) < 0.05
...
for pt in corners[:,0,:]:
  cv2.circle(img_overlay, tuple(pt), 1, (255,255,0),2)
